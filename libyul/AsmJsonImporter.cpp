...
Identifier AsmJsonImporter::createIdentifier(Json const& _node)
{
	...
	identifier.name = m_nodeLabelRegistryBuilder.define(member(_node, "name").get<std::string>());
	return identifier;
}
...
FunctionDefinition AsmJsonImporter::createFunctionDefinition(Json const& _node)
{
	...
	funcDef.name = m_nodeLabelRegistryBuilder.define(member(_node, "name").get<std::string>());

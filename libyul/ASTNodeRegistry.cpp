...
ASTNodeRegistryBuilder::ASTNodeRegistryBuilder(ASTNodeRegistry const& _existingLabels)
{
	for (size_t i = 2; i <= _existingLabels.maximumId(); ++i)
	{
		auto const existingLabel = _existingLabels(i);
		if (!existingLabel.empty())
		{
			auto const [_, inserted] = m_mapping.tryInsert(_existingLabels(i), i);
			yulAssert(inserted);
		}
	}
	m_nextNameId = _existingLabels.maximumId() + 1;
}
